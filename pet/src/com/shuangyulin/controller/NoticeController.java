...
public class NoticeController extends BaseController {
	...
	public void list(String noticeTitle,String noticeClass,String addDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (noticeTitle == null) noticeTitle = "";
		if (noticeClass == null) noticeClass = "";
		if (addDate == null) addDate = "";
		if(rows != 0)noticeService.setRows(rows);
		List<Notice> noticeList = noticeService.queryNotice(noticeTitle, noticeClass, addDate, page);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(noticeTitle, noticeClass, addDate);
	    /*获取到总的页码数目*/
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(Notice notice:noticeList) {
			...
			jsonNotice.accumulate("noticeTitle", notice.getNoticeTitle());
			jsonArray.put(jsonNotice);
		}
	...
	public String frontlist(String noticeTitle,String noticeClass,String addDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (noticeTitle == null) noticeTitle = "";
		if (noticeClass == null) noticeClass = "";
		if (addDate == null) addDate = "";
		List<Notice> noticeList = noticeService.queryNotice(noticeTitle, noticeClass, addDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    noticeService.queryTotalPageAndRecordNumber(noticeTitle, noticeClass, addDate);
	    /*获取到总的页码数目*/
	    int totalPage = noticeService.getTotalPage();
	...
	public void OutToExcel(String noticeTitle,String noticeClass,String addDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(noticeTitle == null) noticeTitle = "";
        if(noticeClass == null) noticeClass = "";
        if(addDate == null) addDate = "";
        List<Notice> noticeList = noticeService.queryNotice(noticeTitle,noticeClass,addDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Notice信息记录"; 
        String[] headers = { "公告id","标题","公告类别","发布日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<noticeList.size();i++) {
        	Notice notice = noticeList.get(i); 
        	dataset.add(new String[]{notice.getNoticeId() + "",notice.getNoticeTitle(),notice.getNoticeClass(),notice.getAddDate()});
        }
        /*
